...
%left TOK_ARRAY TOK_FIELD TOK_FUNC
%left '[' '.'
%nonassoc '('
...
structdef: TOK_STRUCT IDENT '{' '}'   
                                 { $$ = $1->adopt_sym2($2, 
                                 TOK_TYPEID);} 
    |TOK_STRUCT IDENT  fielddecls '}'             
                                 { $$ = $1->adopt_sym2($2, TOK_TYPEID); 
                                 $$= $1 -> adopt($3);}
    
    ;
fielddecls: '{' fielddecl ';'    { $$ = $1->adopt($2); }
    |fielddecls  fielddecl ';'                   
                                 { $$ = $1->adopt($2); }
    
    ;
fielddecl: basetype TOK_ARRAY IDENT                  
...
function : identdecl '(' ')' ';'                            
    ...
    ;
identdecls: '(' identdecl       { $$ = $1->adopt_sym($2, TOK_PRAM); }
    | identdecls ',' identdecl                    
                                { $$ = $1->adopt($3); }
    
    ;
identdecl: basetype TOK_ARRAY IDENT                  
                                { $$ = $1->adopt_sym2($2, TOK_DECLID); 
                                $$ = $1->adopt($2); }
    | basetype IDENT            { $$ = $1->adopt_sym2($2, TOK_DECLID);}
    ;
block: '{' '}'                  { $$ = $$->adopt_sym2($1, TOK_BLOCK);} 
    |statements '}'             { $$ = $$->adopt_sym2($1, TOK_BLOCK);}
    ;
...
call: exprs ')'                 { $$ = $1; }
    ...
    ;
exprs: IDENT '(' expr           { $$ = $2->adopt_sym($1, TOK_CALL); 
                                $$=$2->adopt($3);}
