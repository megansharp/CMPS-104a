...
fielddecl: basetype TOK_ARRAY TOK_IDENT                  
                                 ...
                                 $$=$2->adopt($1);}
    | basetype TOK_IDENT         { $$ = $1->adopt_sym2($2, TOK_FIELD);}
    ;
basetype: TOK_VOID               { $$ = $1; }
     | TOK_INT                   { $$ = $1; }
     | TOK_STRING                { $$ = $1; }
     | TOK_CHAR                  { $$ = $1; }
     | TOK_IDENT                 { $$ = swap_symb($1, TOK_TYPEID);}
     ;
...
identdecl: basetype TOK_ARRAY TOK_IDENT                  
                                { $$ = $2->adopt_sym2($3, TOK_DECLID); 
                                $$ = $2->adopt($1); }
    | basetype TOK_IDENT        { $$ = $1->adopt_sym2($2, TOK_DECLID);}
    ;
block: '{' '}'                  { $$ = swap_symb($1, TOK_BLOCK);} 
    | statements '}'            { $$ = $1; }
    ;
...
call: exprs ')'                 { $$ = $1; }
    ...
    ;
exprs: exprs ',' expr           { $$ = $1->adopt($3);}
    | TOK_IDENT '(' expr        { $$ = $2->adopt_sym($1, TOK_CALL); 
                                $$=$2->adopt($3);}
    ;
variable: TOK_IDENT             { $$ = $1; }
    | expr '[' expr ']'         { $$ = $2->adopt_sym($3, TOK_INDEX); 
                                $$=$2->adopt($1); }
    | expr '.' TOK_IDENT        { $$ = $2->adopt_sym2($3,TOK_FIELD);
                                $$ = $2-> adopt($1); }
    ;
